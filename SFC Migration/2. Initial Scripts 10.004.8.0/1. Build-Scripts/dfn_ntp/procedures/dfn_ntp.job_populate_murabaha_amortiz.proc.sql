CREATE OR REPLACE PROCEDURE dfn_ntp.job_populate_murabaha_amortiz
IS
...
BEGIN
    SELECT v07_id
      ...
      FROM v07_db_jobs
     WHERE v07_job_name = 'POPULATE_MURABAHA_AMORTIZE';
    ...
    THEN
        sp_populate_murabaha_amortize ();

        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            1,
            'POPULATE_MURABAHA_AMORTIZE Executed Successfully');
    ELSE
        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            ...
            || l_dependant_job
            || ' Not Executed! Current Job POPULATE_MURABAHA_AMORTIZE Will Not Execute');
...
EXCEPTION
    ...
    THEN
        ...
        job_sch_send_notification (
               'ERROR!!! POPULATE_MURABAHA_AMORTIZE Execution Failed -> ENABLE_TRANSACTION'
            || SUBSTR (SQLERRM, 1, 512));
END;
/