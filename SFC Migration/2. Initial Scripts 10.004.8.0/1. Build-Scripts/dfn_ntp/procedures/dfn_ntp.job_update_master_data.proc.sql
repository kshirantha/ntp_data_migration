CREATE OR REPLACE PROCEDURE dfn_ntp.job_update_master_data
IS
...
BEGIN
    SELECT v07_id
      ...
      FROM v07_db_jobs
     WHERE v07_job_name = 'UPDATE_MASTER_DATA';
    ...
    THEN
        --sp_update_master_data (); stopped in SFC due to function removed
        -- COMMIT;

        sp_populate_forex_history ();
        COMMIT;

        sp_update_eom_tax_invoices ();
        COMMIT;

        sp_populate_int_ind_history ();
        COMMIT;
        ...
        job_sch_update_job_history (
            ...
            1,
            'UPDATE_MASTER_DATA Executed Successfully');
    ELSE
        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            ...
            || l_dependant_job
            || ' Not Executed! Current Job UPDATE_MASTER_DATA Will Not Execute');
...
EXCEPTION
    ...
    THEN
        ...
        job_sch_send_notification (
               'ERROR!!! UPDATE_MASTER_DATA Execution Failed -> '
            || SUBSTR (SQLERRM, 1, 512));
END;
/