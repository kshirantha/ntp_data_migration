CREATE OR REPLACE PROCEDURE dfn_ntp.job_update_omini_bank_balances
IS
...
BEGIN
    SELECT v07_id
      ...
      FROM v07_db_jobs
     WHERE v07_job_name = 'UPDATE_OMINI_BANK_BALANCES';
    ...
    THEN
        FOR c_branch IN (SELECT m02_id
                         FROM m02_institute)
        LOOP
            sp_update_omini_bank_balances (pdate     => SYSDATE,
                                           pinstid   => c_branch.m02_id,
                                           peod      => 0);
            COMMIT;
        END LOOP;
        ...
        job_sch_update_job_history (
            ...
            1,
            'UPDATE_OMINI_BANK_BALANCES Executed Successfully');
    ELSE
        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            ...
            || l_dependant_job
            || ' Not Executed! Current Job UPDATE_OMINI_BANK_BALANCES Will Not Execute');
...
EXCEPTION
    ...
    THEN
        ...
        job_sch_send_notification (
               'ERROR!!! UPDATE_OMINI_BANK_BALANCES Execution Failed -> '
            || SUBSTR (SQLERRM, 1, 512));
END;
/