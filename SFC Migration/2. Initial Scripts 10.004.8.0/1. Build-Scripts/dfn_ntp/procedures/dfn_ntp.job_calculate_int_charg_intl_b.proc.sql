CREATE OR REPLACE PROCEDURE dfn_ntp.job_calculate_int_charg_intl_b
IS
...
BEGIN
    SELECT v07_id
      ...
      FROM v07_db_jobs
     WHERE v07_job_name = 'CALCULATE_INT_CHARGES_INTL';
    ...
    THEN
        ...
        sp_calc_incident_od_interest_b (ptype => 1);

        sp_calc_incident_with_intrst_b (ptype => 1);

        sp_calculate_margin_interest_b (ptype => 1);
        ...
        job_sch_update_job_history (
            ...
            1,
            'CALCULATE_INT_CHARGES_INTL Executed Successfully');
    ELSE
        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            ...
            || l_dependant_job
            || ' Not Executed! Current Job CALCULATE_INT_CHARGES_INTL Will Not Execute');
...
EXCEPTION
    ...
    THEN
        ...
        job_sch_send_notification (
               'ERROR!!! CALCULATE_INT_CHARGES_INTL Execution Failed -> '
            || SUBSTR (SQLERRM, 1, 512));
END;
/