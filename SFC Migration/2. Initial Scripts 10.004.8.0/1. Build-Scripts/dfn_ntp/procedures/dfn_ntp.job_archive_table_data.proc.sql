CREATE OR REPLACE PROCEDURE dfn_ntp.job_archive_table_data
IS
...
BEGIN
    SELECT v07_id
      ...
      FROM v07_db_jobs
     WHERE v07_job_name = 'ARCHIVE_TABLE_DATA';
    ...
    THEN
        dfn_ntp.sp_arc_set_archive_ready ();
        COMMIT;

        dfn_ntp.sp_arc_archive_data ();
        COMMIT;
        ...
        job_sch_update_job_history (
            ...
            1,
            'ARCHIVE_TABLE_DATA Executed Successfully');
    ELSE
        job_sch_update_job_history (
            l_current_job_id,
            l_current_job_log_id,
            ...
            || l_dependant_job
            || ' Not Executed! Current Job ARCHIVE_TABLE_DATA Will Not Execute');
...
EXCEPTION
    ...
    THEN
        ...
        job_sch_send_notification (
               'ERROR!!! ARCHIVE_TABLE_DATA Execution Failed -> '
            || SUBSTR (SQLERRM, 1, 512));
END;
/