CREATE OR REPLACE FORCE VIEW dfn_ntp.vw_u01_customer_list_file
(
   u01_id,
   u01_customer_no,
...
AS
   SELECT u01_id,
      ...
      FROM u01_customer
           ...
           LEFT JOIN (  SELECT LISTAGG (u07_exchange_account_no, ',')
                               WITHIN GROUP (ORDER BY u07_exchange_account_no)
                               AS u07_exchange_account_list,
                           u07_customer_id_u01
                      FROM u07_trading_account
                  GROUP BY u07_customer_id_u01) u07_exg_acc
			   ON u01_id = u07_exg_acc.u07_customer_id_u01
/
